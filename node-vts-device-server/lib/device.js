...
function Device(adapter, connection, gpsServer) {
  ...
  this.ping = function (msgParts) {
    ...
    _this.do_log('GPS DATA'+ gpsData);
    gpsData.from_cmd = msgParts.cmd;
    gpsData.device_id=this.getUID();
    _this.emit('ping', gpsData, msgParts);

  };

  this.status = function(msgParts){
    var status = this.adapter.get_status(msgParts);

    _this.emit('status',status,msgParts);
  }
