...
exports.get_distance = function(p1, p2) {
  ...
  var a =
    ...
    Math.cos(exports.rad(p1.lat)) *
      Math.cos(exports.rad(p2.lat)) *
      Math.sin(dLong / 2) *
      Math.sin(dLong / 2);
...
exports.minute_to_decimal = function(pos, pos_i) {
  ...
  return pos_i.toUpperCase() === "S" || pos_i.toUpperCase() === "W"
    ? res * -1
    : res;
};
...
exports.invalid_data = function(data) {
  var d = new Date();
...
exports.in_bd = function(fire_date) {
  if (
    fire_date.lat > 20 &&
    fire_date.lat < 27 &&
    fire_date.lng >= 88 &&
...
/**
 * This method is used to get orientation of device to show 
 * vehicle in right position in degree when it turns left right and reverse and so on
 ...
 * if 2 bytes in hexa converted into binary it returns 16 digits of binary number
 * for example hexa number is c541, if we convert it into binary it returns 1100 0101 0100 0001
 * 9 bits from right side represent orientation in degrees, in this case  1 0100 0001 return 321 deg
 * @param {hex value} orientationInHex 
 * @returns integer
 */
exports.getOrientation=function(orientationInHex){
  const orientationInBinary=(parseInt(orientationInHex, 16).toString(2)).padStart(8, '0');
  return parseInt(orientationInBinary.substr(7,16),2);
}